...
function Home() {
  ...
  const movies = moviesApi.map((data, i) => {
    ...
    const overviewCut = data.overview.substr(0, 249) + '...'
    const isLiked = likedMovies.some(movie => movie === data.title);
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={overviewCut} poster={path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
